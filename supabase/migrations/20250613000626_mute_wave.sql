/*
  ...
  1. New Tables
    - `saved_searches` - User saved search criteria and job alerts
    - `job_alerts` - Tracks job matches for saved searches
    - `search_analytics` - Search behavior tracking and analytics

  2. Search Functions
    - `calculate_skill_match_score` - Calculates skill compatibility percentage
    - `search_jobs` - Advanced job search with multiple filters
    - `search_professionals` - Professional search with skill matching
    - `check_job_alerts` - Automated job alert checking
    - `log_search_analytics` - Search behavior logging
...
-- Function to create searchable text for jobs
CREATE OR REPLACE FUNCTION job_search_text(
  title text,
  description text,
  required_skills text[]
)
RETURNS text AS $$
BEGIN
  RETURN title || ' ' || description || ' ' || coalesce(array_to_string(required_skills, ' '), '');
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Function to create searchable text for professionals
CREATE OR REPLACE FUNCTION professional_search_text(
  title text,
  skills text[]
)
RETURNS text AS $$
BEGIN
  RETURN coalesce(title, '') || ' ' || coalesce(array_to_string(skills, ' '), '');
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Add search text columns to existing tables
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'jobs' AND column_name = 'search_text'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'professional_profiles' AND column_name = 'search_text'
  ) THEN
    ALTER TABLE professional_profiles ADD COLUMN search_text text;
  END IF;
END $$;

-- Update search text for existing records
UPDATE jobs SET search_text = job_search_text(title, description, required_skills);
UPDATE professional_profiles SET search_text = professional_search_text(title, skills);

-- Create triggers to maintain search text
CREATE OR REPLACE FUNCTION update_job_search_text()
RETURNS TRIGGER AS $$
BEGIN
  NEW.search_text = job_search_text(NEW.title, NEW.description, NEW.required_skills);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION update_professional_search_text()
RETURNS TRIGGER AS $$
BEGIN
  NEW.search_text = professional_search_text(NEW.title, NEW.skills);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Drop existing triggers if they exist
DROP TRIGGER IF EXISTS update_job_search_text_trigger ON jobs;
DROP TRIGGER IF EXISTS update_professional_search_text_trigger ON professional_profiles;

-- Create new triggers
CREATE TRIGGER update_job_search_text_trigger
  BEFORE INSERT OR UPDATE ON jobs
  FOR EACH ROW
  EXECUTE FUNCTION update_job_search_text();

CREATE TRIGGER update_professional_search_text_trigger
  BEFORE INSERT OR UPDATE ON professional_profiles
  FOR EACH ROW
  EXECUTE FUNCTION update_professional_search_text();

-- Full-text search indexes using the search_text columns
CREATE INDEX IF NOT EXISTS idx_jobs_search_text ON jobs USING gin(to_tsvector('english', search_text));
CREATE INDEX IF NOT EXISTS idx_professional_profiles_search_text ON professional_profiles USING gin(to_tsvector('english', search_text));
  ...
  SELECT 
    ...
    CASE 
      WHEN p_query IS NOT NULL THEN
        ts_rank(to_tsvector('english', j.search_text), plainto_tsquery('english', p_query))
      ELSE 1.0
    END as relevance_score
  FROM jobs j
  JOIN profiles p ON p.id = j.hirer_id
  WHERE j.status = 'active'
    AND (p_query IS NULL OR to_tsvector('english', j.search_text) @@ plainto_tsquery('english', p_query))
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN p_query IS NOT NULL THEN
        ts_rank(to_tsvector('english', pp.search_text), plainto_tsquery('english', p_query))
      ELSE 1.0
    END as relevance_score
  FROM professional_profiles pp
  JOIN profiles p ON p.id = pp.user_id
  WHERE (p_query IS NULL OR to_tsvector('english', pp.search_text) @@ plainto_tsquery('english', p_query))
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Create notification (assuming this function exists)

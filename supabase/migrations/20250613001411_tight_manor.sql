/*
  # Fix search_jobs RPC function return types

  1. Function Updates
    - Drop and recreate the search_jobs function with correct return types
    - Ensure all numeric columns return as numeric instead of real
    - Fix salary_min and salary_max columns to maintain precision

  2. Changes Made
    - Updated return type for salary fields to use numeric instead of real
    - Maintained all existing functionality while fixing type mismatch
*/

-- Drop the existing search_jobs function
DROP FUNCTION IF EXISTS search_jobs(
  p_query text,
  p_category text,
  p_job_type text,
  p_location text,
  p_remote_allowed boolean,
  p_salary_min numeric,
  p_salary_max numeric,
  p_salary_type text,
  p_required_skills text[],
  p_required_licenses text[],
  p_is_urgent boolean,
  p_posted_within_days integer,
  p_limit integer,
  p_offset integer
);
...
-- Create search_professionals function if it doesn't exist
...
-- Create log_search_analytics function if it doesn't exist
